    ...
    while IFS= read -r line; do
        ...
        if grep -Eq '^[[:space:]]*[a-zA-Z0-9_-]+:[[:space:]]*([^{].+)?$' <<< "$line"; then
            ...
            if ! grep -Eq '{{.*}}' <<< "$line" && grep -Eq ':[[:space:]]*"[^"]+"' <<< "$line"; then
                echo "Warning: Hard-coded string value found in $file_name: $line"
                hard_coded_values_found=true
            fi
            if ! grep -Eq '{{.*}}' <<< "$line" && grep -Eq ':[[:space:]]*[0-9]+' <<< "$line"; then
                echo "Warning: Hard-coded numeric value found in $file_name: $line"
                hard_coded_values_found=true
            fi
            if ! grep -Eq '{{.*}}' <<< "$line" && grep -Eq ':[[:space:]]*[a-zA-Z0-9_-]+' <<< "$line"; then
                echo "Warning: Hard-coded key-value pair found in $file_name: $line"
                hard_coded_values_found=true
            fi
        fi
    done < "$file_path"
