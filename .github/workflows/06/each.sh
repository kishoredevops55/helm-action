...
# Function to check if a file contains hard-coded values
check_hard_coded_values() {
    ...
    while IFS= read -r line; do
        ((line_number++))
        if grep -Eq ':[[:space:]]*[0-9]+' <<< "$line" || grep -Eq ':[[:space:]]*".+"' <<< "$line"; then
            echo "Warning: Hard-coded value found at line $line_number in $file_path"
