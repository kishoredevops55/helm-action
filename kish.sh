...
# Extract the potential controller name and ordinal from the PVC name
# Assuming the PVC name format is "data-<controller-name>-<ordinal>"
PREFIX="data-"
SUFFIX=$(echo "$PVC_NAME" | sed "s/^$PREFIX//")
CONTROLLER_NAME=$(echo "$SUFFIX" | rev | cut -d'-' -f2- | rev)
ORDINAL=$(echo "$SUFFIX" | rev | cut -d'-' -f1 | rev)

# Debug output
echo "Extracted controller name: $CONTROLLER_NAME"
echo "Extracted ordinal: $ORDINAL"
...
# Check StatefulSets
STATEFULSETS=$(kubectl get statefulsets -n $NAMESPACE -o json | jq -r ".items[] | select(.metadata.name | contains(\"$CONTROLLER_NAME\")) | .metadata.name")
...
DEPLOYMENTS=$(kubectl get deployments -n $NAMESPACE -o json | jq -r ".items[] | select(.metadata.name | contains(\"$CONTROLLER_NAME\")) | .metadata.name")
